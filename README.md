...
- `nestedOptions` - an object where each key is a property, and its value is an object of options (`pruneMethod`, `replaceValue`, and `stripNull`). If that field is an object, the options value overrides the default options for each of its nested fields.
